...
@section('title', $currentFolder ? $currentFolder->name : 'Explorateur de Fichiers')
...
<div class="space-y-8">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 bg-surface-container-low dark:bg-slate-900/50 p-6 rounded-2xl ring-1 ring-outline-variant/10 dark:ring-slate-800">
        <div class="space-y-1">
            <div class="flex items-center gap-3">
                <h2 class="text-xl font-black text-on-surface dark:text-white tracking-tight uppercase">Archives</h2>
                @if(request('owner') === 'me')
                    <span class="px-3 py-1 bg-primary text-white text-[9px] font-black uppercase tracking-[0.2em] rounded-full shadow-lg shadow-primary/20">Mes fichiers</span>
                @endif
            </div>
            <nav class="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-outline dark:text-slate-500 flex-wrap">
                ...
                @foreach($breadcrumbs as $bc)
                    ...
                    @else
                        <a href="{{ route('admin.explorer', $bc->id) }}" 
                           ondragover="event.preventDefault(); this.classList.add('text-primary', 'underline')" 
                           ondragleave="this.classList.remove('text-primary', 'underline')" 
        ...
        <div class="flex items-center gap-3">
            <button onclick="document.getElementById('modal-folder').classList.remove('hidden')"
                ...
                <span class="material-symbols-outlined text-lg">create_new_folder</span> Nouveau dossier
            </button>
            <button onclick="document.getElementById('modal-upload').classList.remove('hidden')"
                ...
                <span class="material-symbols-outlined text-lg">cloud_upload</span> Transférer
            </button>
        </div>
    </div>
    ...
    <div class="space-y-4">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
            ...
            <div ondragover="event.preventDefault(); this.classList.add('ring-2', 'ring-primary', 'bg-primary/5')" 
                ...
                <a href="{{ $currentFolder->parent_id ? route('admin.explorer', $currentFolder->parent_id) : route('admin.explorer') }}" class="flex flex-col items-center gap-3">
            ...
            <div data-folder-id="{{ $folder->id }}" 
                ...
                <a href="{{ route('admin.explorer', $folder->id) }}" class="flex flex-col items-center gap-3">
                ...
                <div class="absolute top-2 right-2 hidden group-hover:flex gap-1 z-10">
                    ...
                    <button type="button" onclick="openDeleteModal('{{ route('admin.folders.destroy', $folder->id) }}', true)"
    ...
    <div class="space-y-4 pt-4">
        <div class="flex items-center gap-3">
            ...
            <h3 class="text-[10px] font-black text-outline dark:text-slate-500 uppercase tracking-widest">Fichiers ({{ $files->count() }})</h3>
        </div>
        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
            ...
            <div draggable="true" ondragstart="handleDragStart(event, {{ $file->id }}, '{{ $file->name }}')"
                ...
                <a href="{{ route('admin.files.show', $file) }}" class="flex flex-col items-center gap-3">
                    ...
                    <div class="text-center w-full min-w-0">
                        ...
                        <p class="text-[10px] font-black text-outline dark:text-slate-500 truncate">{{ strtoupper($file->extension) }} • {{ number_format($file->size / 1024 / 1024, 2) }} MO</p>
                    </div>
                </a>
                {{-- Actions au survol --}}
                <div class="absolute top-2 right-2 hidden group-hover:flex gap-1 z-10">
                    <a href="{{ route('admin.files.permissions', $file) }}" class="p-1.5 bg-primary/10 dark:bg-blue-400/10 text-primary dark:text-blue-400 rounded-lg hover:bg-primary hover:text-white transition-all" title="Permissions">
                        <span class="material-symbols-outlined text-sm">lock_open</span>
                    </a>
                    <button type="button" onclick="openDeleteModal('{{ route('admin.files.destroy', $file->id) }}', false)" 
                        class="p-1.5 bg-rose-500/10 text-rose-500 rounded-lg hover:bg-rose-500 hover:text-white transition-all" title="Supprimer">
    ...
    <div class="flex flex-col items-center justify-center py-32 space-y-4">
        ...
        <div class="text-center">
            ...
            <p class="text-sm text-outline dark:text-slate-500">Commencez par créer un dossier ou par transférer un fichier.</p>
        </div>
    </div>
    @endif
</div>
...
<div id="modal-folder" class="hidden fixed inset-0 bg-on-surface/40 backdrop-blur-sm z-[60] flex items-center justify-center p-4">
    <div class="bg-surface-container-lowest dark:bg-slate-900 rounded-3xl p-8 w-full max-w-md shadow-2xl ring-1 ring-outline-variant/10">
        ...
        <form action="{{ route('admin.folders.store') }}" method="POST" class="space-y-5">
            @csrf
...
async function confirmMove() {
    ...
    try {
        const response = await fetch(`/admin/files/${draggedFile.id}/move`, {
            method: 'PATCH',
...
function openRenameModal(folderId, currentName) {
    ...
    form.action = `/admin/folders/${folderId}`;
