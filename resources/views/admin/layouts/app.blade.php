...
<head>
    ...
    <title>Share File - @yield('title')</title>
...
<body
    ...
    x-data="{ 
      ...
      }" x-init="document.documentElement.classList.remove('sidebar-is-collapsed')">
    ...
    <div class="flex h-screen overflow-hidden">
                ...
                <div>
                    ...
                    <div class="space-y-1">
                        <a href="{{ route('admin.dashboard') }}"
                            class="group flex items-center {{ request()->routeIs('admin.dashboard') ? 'bg-primary text-white shadow-md shadow-primary/20' : 'text-slate-500 dark:text-slate-400 hover:bg-white dark:hover:bg-slate-800' }} transition-all duration-200 rounded-2xl"
                            :class="sidebarCollapsed ? 'justify-center p-3' : 'gap-3 px-4 py-3'">
                            <span
                                class="material-symbols-outlined text-lg {{ request()->routeIs('admin.dashboard') ? '' : 'group-hover:text-primary dark:group-hover:text-blue-400' }}">dashboard</span>
                ...
                <div>
                    ...
                    <div class="space-y-1">
                        <a href="{{ route('admin.users.index') }}"
                            class="group flex items-center {{ request()->routeIs('admin.users*') ? 'bg-primary text-white shadow-md shadow-primary/20' : 'text-slate-500 dark:text-slate-400 hover:bg-white dark:hover:bg-slate-800' }} transition-all duration-200 rounded-2xl"
                            :class="sidebarCollapsed ? 'justify-center p-3' : 'gap-3 px-4 py-3'">
                            <span
                                class="material-symbols-outlined text-lg {{ request()->routeIs('admin.users*') ? '' : 'group-hover:text-primary dark:group-hover:text-blue-400' }}">group</span>
                            <span x-show="!sidebarCollapsed"
                                class="font-bold text-xs uppercase tracking-widest whitespace-nowrap">Utilisateurs</span>
                        </a>
                        <a href="{{ route('admin.admins.index') }}"
                            class="group flex items-center {{ request()->routeIs('admin.admins*') ? 'bg-primary text-white shadow-md shadow-primary/20' : 'text-slate-500 dark:text-slate-400 hover:bg-white dark:hover:bg-slate-800' }} transition-all duration-200 rounded-2xl"
                            :class="sidebarCollapsed ? 'justify-center p-3' : 'gap-3 px-4 py-3'">
                            <span
                                class="material-symbols-outlined text-lg {{ request()->routeIs('admin.admins*') ? '' : 'group-hover:text-primary dark:group-hover:text-blue-400' }}">admin_panel_settings</span>
                            <span x-show="!sidebarCollapsed"
                                class="font-bold text-xs uppercase tracking-widest whitespace-nowrap">Admins</span>
                        </a>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="space-y-1">
                        <a href="{{ route('admin.audit.index') }}"
                            class="group flex items-center {{ request()->routeIs('admin.audit*') ? 'bg-primary text-white shadow-md shadow-primary/20' : 'text-slate-500 dark:text-slate-400 hover:bg-white dark:hover:bg-slate-800' }} transition-all duration-200 rounded-2xl"
                            :class="sidebarCollapsed ? 'justify-center p-3' : 'gap-3 px-4 py-3'">
                            <span
                                class="material-symbols-outlined text-lg {{ request()->routeIs('admin.audit*') ? '' : 'group-hover:text-primary dark:group-hover:text-blue-400' }}">assignment</span>
                            <span x-show="!sidebarCollapsed"
                                class="font-bold text-xs uppercase tracking-widest whitespace-nowrap">Audit Logs</span>
                        </a>
                    </div>
                </div>
            </nav>
        ...
        <div class="flex-1 flex flex-col h-full overflow-hidden bg-surface dark:bg-slate-950">
            ...
            <div class="px-4 sm:px-8 pt-4 pb-2">
                <header
                    ...
                    <div class="flex items-center gap-2 sm:gap-4">
                        ...
                        <!-- Séparateur + user info (visible >= xl seulement) -->
                        <div class="hidden xl:flex items-center gap-3">
                            <div class="w-[1px] h-6 bg-outline-variant/20 dark:bg-slate-800"></div>
                            <span class="text-xs font-bold text-on-surface dark:text-white truncate max-w-[150px]">{{ auth()->user()->name }}</span>
                            <div class="w-8 h-8 rounded-xl overflow-hidden shadow-md ring-2 ring-white dark:ring-slate-900">
                                <img src="https://ui-avatars.com/api/?name={{ urlencode(auth()->user()->name) }}&background=00488d&color=fff"
                                    class="w-full h-full object-cover">
                            </div>
                        </div>
            ...
            <div class="flex-1 overflow-y-auto px-10 pt-4">
    ...
    <script>
        ...
        function showToast(message, type = 'success') {
            ...
            toast.className = `toast-animate flex items-center gap-4 bg-white dark:bg-slate-900 p-4 pr-6 rounded-2xl shadow-2xl border-l-4 border-${color}-500 ring-1 ring-black/5 min-w-[300px]`;
            toast.innerHTML = `
                <div class="w-10 h-10 rounded-full bg-${color}-50 dark:bg-${color}-900/20 flex items-center justify-center text-${color}-500">
                    <span class="material-symbols-outlined">${icon}</span>
                </div>
                <div class="flex-1">
                    <p class="text-sm font-black text-on-surface dark:text-white">${message}</p>
                    <p class="text-[10px] font-medium text-outline dark:text-slate-500 uppercase tracking-widest">Système</p>
                </div>
            `;
