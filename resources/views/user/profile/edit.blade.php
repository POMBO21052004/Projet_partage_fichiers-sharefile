...
<div class="max-w-4xl mx-auto">
    ...
    <div class="mb-8">
        <nav class="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-outline dark:text-slate-500 mb-3">
            <a href="{{ route('user.profile.show') }}" class="cursor-pointer hover:text-primary transition-colors">Mon Profil</a>
        ...
        <div class="flex items-end gap-6">
            ...
            <div class="flex-1">
                ...
                <p class="text-outline dark:text-slate-500 text-sm font-medium">Mettez à jour vos informations personnelles et sécurisez vos accès.</p>
            </div>
        </div>
    </div>
    ...
    <form action="{{ route('user.profile.update') }}" method="POST" class="grid grid-cols-1 md:grid-cols-3 gap-8">
        @csrf
        @method('PUT')
        ...
        <div class="md:col-span-2 space-y-6">
            <div class="bg-white dark:bg-slate-900 p-8 rounded-xl shadow-sm border border-black/5 dark:border-white/5">
                ...
                <div class="space-y-6">
                    <div class="grid grid-cols-1 gap-2">
                        <label class="text-[10px] font-black uppercase tracking-widest text-outline dark:text-slate-500 px-1">Nom d'Utilisateur</label>
                    ...
                    <div class="grid grid-cols-1 gap-2">
                        ...
                        <div class="relative">
                            <input class="w-full px-5 py-4 bg-slate-50 dark:bg-slate-800/50 border-none rounded-xl text-sm font-bold text-outline dark:text-slate-600 cursor-not-allowed outline-none" readonly type="email" value="{{ $user->email }}"/>
                    ...
                    <div class="pt-6 border-t border-black/5 dark:border-white/5 space-y-6">
                        <div class="grid grid-cols-1 gap-2">
                            <label class="text-[10px] font-black uppercase tracking-widest text-outline dark:text-slate-500 px-1">Nouveau Mot de Passe</label>
            ...
            <div class="flex items-center justify-end gap-4 pt-4">
                <a href="{{ route('user.profile.show') }}" class="px-8 py-4 rounded-xl text-[10px] font-black uppercase tracking-widest text-outline hover:bg-surface-container-high transition-colors">
                    Annuler
                </a>
            ...
            <div class="bg-primary p-8 rounded-xl shadow-xl shadow-primary/20 relative overflow-hidden group">
                <div class="relative z-10 space-y-4">
                    <div class="flex items-center gap-3 text-white/90">
                        ...
                        <span class="font-black text-xs uppercase tracking-[0.2em]">Sécurité</span>
                    </div>
                    <p class="text-[10px] text-white/80 font-bold uppercase tracking-widest leading-relaxed">
                        Toute modification de mot de passe sera immédiatement appliquée et sécurisera vos documents d'archivage.
                    </p>
                </div>
                <span class="material-symbols-outlined absolute -right-4 -bottom-4 text-7xl text-white/10 select-none group-hover:scale-110 transition-transform duration-700">lock</span>
            </div>

            <div class="bg-surface-container-low dark:bg-slate-900/50 p-6 rounded-xl border border-black/5 dark:border-white/5">
                <h3 class="text-[10px] font-black uppercase tracking-widest text-outline dark:text-slate-500 mb-6">Résumé de mes accès</h3>
                <div class="space-y-4">
                    <div class="flex items-center gap-3">
                        <div class="w-4 h-4 rounded-full bg-emerald-500"></div>
                        <span class="text-[10px] font-black text-on-surface dark:text-white uppercase tracking-widest">Espace Personnel</span>
                    </div>
                    <div class="flex items-center gap-3">
                        <div class="w-4 h-4 rounded-full bg-emerald-500"></div>
                        <span class="text-[10px] font-black text-on-surface dark:text-white uppercase tracking-widest">Dépôt & Lecture</span>
                    </div>
                    <div class="flex items-center gap-3">
                        <div class="w-4 h-4 rounded-full bg-emerald-500"></div>
                        <span class="text-[10px] font-black text-on-surface dark:text-white uppercase tracking-widest">Partage Reçu</span>
                    </div>
                </div>
            </div>
        </div>
